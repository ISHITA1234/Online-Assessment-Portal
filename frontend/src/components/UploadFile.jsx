...
export function UploadFile({ day, question}){
    ...
    //------------------------------------------------------------------------------------------------------------------------
    //############################################## 3. Defining useEffect Hook #########################################
    //------------------------------------------------------------------------------------------------------------------------
    //only when all the state variables are having values updated then only call calculate marks function
    useEffect(() => {
      console.log()
      if (
        userId != "" &&
        selectedFilePath != "" &&
        !(Object.values(testcase).every(value => value === "")) &&
        !(Object.values(output).every(value => value === ""))
      ) {
        calculateMarks(); //CALL MARKS 
      }
    }, [testcase,output,userId,selectedFilePath]);
    ...
    async function calculateMarks() {             //testcase,output,userId,selectedFilePath
      let marksgot = 0;
      ...
      for (let i = 1; i < 11; i++) {              
        if(testcase["testcase"+i]!="")
        {
          if(output["output"+i]==testcase["testcase"+i])
            {
              testcases_passed = testcases_passed + 1;
            }  
          else if(output["output"+i]==testcase["testcase"+i].split("Output:")[1])
            {
              testcases_passed = testcases_passed + 1;
            }        
        }
  ...
  const handleUpload = () => {
      ...
      //------------------------------------------------------------------------------------------------------------------------
      //############################## 6. TestCases fetch for this question #########################
      //------------------------------------------------------------------------------------------------------------------------
